/* ==============================================
      ...
      if (!admin) {
        ...
        document.getElementById("admin-nav-name").textContent = "👤 " + (admin.name || admin.email);
      }
      ...
      addMedForm.addEventListener("submit", async function (e) {
        ...
        try {
          ...
          showMsg("med-msg", "error", "❌ Server not reachable. Is the backend running?");
        } finally {
          btn.disabled = false;
      ...
      function getStatusClass(s) {
        const map = { pending:"status-pending", confirmed:"status-confirmed", shipped:"status-shipped", out_for_delivery:"status-shipped", delivered:"status-delivered", cancelled:"status-cancelled" };
      ...
      async function fetchOrders() {
        ...
        try {
          ...
          const orders = raw.slice().sort((a, b) =>
          ...
          if (orders.length === 0) { setOrderState("orders-empty"); return; }
          setOrderState(null);
          renderOrders(orders);
        } catch (err) {
      ...
      function renderOrders(orders) {
        ...
        orders.forEach((order) => {
          ...
          // Medicine pills
          const pillsHtml = (order.items || []).map((item) => {
      ...
      async function updateOrderStatus(orderId, newStatus) {
        ...
        const buttons = orderCard ? orderCard.querySelectorAll('.order-actions button') : [];
        ...
        try {
          ...
          if (result.success) {
            ...
            if (btnConfirm) btnConfirm.disabled = !(currentIndex + 1 === 1 && newStatus !== "cancelled");
            if (btnShip) btnShip.disabled = !(currentIndex + 1 === 2 && newStatus !== "cancelled");
            if (btnOut) btnOut.disabled = !(currentIndex + 1 === 3 && newStatus !== "cancelled");
            if (btnDeliver) btnDeliver.disabled = !(currentIndex + 1 === 4 && newStatus !== "cancelled");
            if (btnCancel) btnCancel.disabled = (newStatus === "delivered" || newStatus === "cancelled");
      ...
      async function fetchMedicinesAdmin() {
        ...
        try {
          ...
          if (medicinesCache.length === 0) { setMedState("med-empty"); return; }
          setMedState(null);
      ...
      async function deleteMedicineAdmin(id, name) {
        ...
        try {
          ...
          if (result.success) {
            ...
            showToast("❌ " + result.message);
            if (delBtn)  { delBtn.disabled  = false; delBtn.textContent  = "🗑️ Delete"; }
      ...
      function openEditModal(id) {
        ...
        if (!med) { showToast("❌ Medicine not found in cache. Refresh."); return; }
        editingId = id;
      ...
      async function saveEditMedicine() {
        ...
        if (!payload.name || !payload.description || !payload.price || !payload.category) {
          ...
          msgEl.textContent = "❌ Name, Description, Price and Category are required.";
